...
Q_values_th = np.array([ 4, 6, 8, 10, 12, 14, 16 ])
...
if doCompute:
	...
	K_corr_cat_th = np.zeros(( 2, len(Q_values_th), len(offset_phi_values_th) ))
	K_corr_ctx_th = np.zeros(( 2, len(Q_values_th), len(offset_phi_values_th) ))
	Y_corr_cat_th = np.zeros(( 2, len(Q_values_th), len(offset_phi_values_th) ))
	Y_corr_ctx_th = np.zeros(( 2, len(Q_values_th), len(offset_phi_values_th) ))
	...
	for i, Q in enumerate(Q_values_th):
...
# line, = plt.plot(Q_values_th, Y_corr_avg_th[0,:,0], color = '#FFD012', label = 'pre-')
# line, = plt.plot(Q_values_th, Y_corr_avg_th[1,:,0], '0.8', label = 'post-')

for j, offset in enumerate(offset_phi_values_th):
	line, = plt.plot(Q_values_th, Y_corr_ctx_th[:,j], '-', color = palette[j])
