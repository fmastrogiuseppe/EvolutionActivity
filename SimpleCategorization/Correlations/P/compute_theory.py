...
P_values_th = np.linspace( 2, 20, 100 ).astype(int)
offset_values_th = np.linspace( 1, 2, 4 )

K_bar_th = np.zeros(( 2, 2, len(P_values_th), len(offset_values_th) ))  # First axis indicates t = 0 or t = T
Y_bar_th = np.zeros(( 2, 2, len(P_values_th), len(offset_values_th) ))

K_corr_th = np.zeros(( 2, len(P_values_th), len(offset_values_th) ))
Y_corr_th = np.zeros(( 2, len(P_values_th), len(offset_values_th) ))
...
if doCompute:
	...
	for j, offset in enumerate(offset_values_th):
		...
		for i, P in enumerate(P_values_th):
	...
	fac.Store(P_values_th, 'P_values_th.p', 'Results/')
	...
	P_values_th = fac.Retrieve('P_values_th.p', 'Results/')
...
# line, = plt.plot(P_values_th, Y_corr_th[0,:,0], color = '#FFD012')
# line, = plt.plot(P_values_th, Y_corr_th[1,:,0], '0.8', label = 'post-')

for j, offset in enumerate(offset_values_th):
	line, = plt.plot(P_values_th, Y_corr_th[1,:,j], color = palette[j])

plt.xlabel(r'Number of stimuli')
